...
import com.lee.biz.search.SearchService;
import com.lee.biz.search.SearchVO;
...
public class SearchController {
	...
	public String carSearch(@RequestParam(value="fuel", required=false) String[] fuel, 
			@RequestParam(value="city", required=false) String[] city, SearchVO svo, Model model, HttpSession session) {
		...
		if(svo.getPrice_min() != 0 && svo.getPrice_max() != 0) {
			...
			System.out.println("가격 최소값 : "+svo.getPrice_min());
			System.out.println("가격 최대값 : "+svo.getPrice_max());
		}
		//연식
		//검색할 때 입력한 값이 null이 아닌경우 = 범위를 위해 값을 입력하고 검색한 경우
		if(svo.getYear_min() != 0 && svo.getYear_max() != 0) {
			...
			System.out.println("연식 최소값 : "+svo.getYear_min());
			System.out.println("연식 최대값 : "+svo.getYear_max());
		}

		//주행거리
		//         System.out.println("형변환 전 : "+request.getParameter("min-value"));
		if(svo.getKm_min() != 0 && svo.getKm_max() != 0) {
			System.out.println("주행거리 최소값 : "+svo.getKm_min());
			System.out.println("주행거리 최대값 : "+svo.getKm_max());
			model.addAttribute("kmin",svo.getKm_min());
			model.addAttribute("kmax", svo.getKm_max());
		}
		// 정렬
		model.addAttribute("sort", svo.getChecksort());
		System.out.println("필터 정렬 상태 : "+session.getAttribute("sort"));
