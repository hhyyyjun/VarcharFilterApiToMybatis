...
const filterSlider = document.getElementById("Slider");
// script태그로 슬라이더 초기값 전달 받기
let kmin = filterSlider.getAttribute("kmin");
let kmax = filterSlider.getAttribute("kmax");
//null값이 아니라 false로 해야함 => js로 가져올 때 데이터가 없으면 null이 아니라 false?
//동적타이핑 , output이 object? boolean이랑 더 근접했기 때문에 false가 나오는 것 같다!
...
console.log(kmax);
  ...
  rangeSlider.noUiSlider.on("update", function (values, handle) {
    ...
    document.querySelector("input[name='km_min']").value = moneyFormat.from(values[0]);
    document.querySelector("input[name='km_max']").value = moneyFormat.from(values[1]);
  	//console.log(document.getElementsByName("min-value").value);
  	//console.log(document.getElementsByName("max-value").value);
  });
});
